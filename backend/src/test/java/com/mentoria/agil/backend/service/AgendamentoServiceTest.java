...
class AgendamentoServiceTest {
        ...
        void deveAgendarComSucesso() {
                ...
                verify(sessaoRepository, times(1))
                                .existsByMentorAndDataHoraInicioLessThanEqualAndDataHoraFimGreaterThanEqual(
        ...
        void deveLancarExcecaoQuandoUsuarioNaoForMentor() {
                ...
                verify(sessaoRepository, never())
                                .existsByMentorAndDataHoraInicioLessThanEqualAndDataHoraFimGreaterThanEqual(any(),
                                                any(), any());
        ...
        void deveLancarExcecaoQuandoMentorNaoDisponivel() {
                ...
                when(sessaoRepository.existsByMentorAndDataHoraInicioLessThanEqualAndDataHoraFimGreaterThanEqual(
                                mentor, dto.getDataHoraFim(), dto.getDataHoraInicio()))
                                .thenReturn(false);
                when(disponibilidadeRepository.findDisponiveisNoIntervalo(mentor, dto.getDataHoraInicio(),
                                dto.getDataHoraFim()))
                                .thenReturn(List.of());
        ...
        @Test
        @DisplayName("Deve falhar quando endereço presencial for apenas espaços")
        void agendarPresencialEnderecoInvalido() {
                when(userRepository.findById(any())).thenReturn(Optional.of(mentor));
                when(sessaoRepository.existsByMentorAndDataHoraInicioLessThanEqualAndDataHoraFimGreaterThanEqual(any(),
        ...
        @Test
        @DisplayName("Deve salvar endereço corretamente em sessões PRESENCIAIS")
        void agendarPresencialSucesso() {
                when(userRepository.findById(any())).thenReturn(Optional.of(mentor));
                when(sessaoRepository.existsByMentorAndDataHoraInicioLessThanEqualAndDataHoraFimGreaterThanEqual(any(),
                                any(), any())).thenReturn(false);
                when(disponibilidadeRepository.findDisponiveisNoIntervalo(any(), any(), any()))
                ...
                dto.setFormato(FormatoSessao.PRESENCIAL);
                dto.setEndereco("Rua Teste, 10");
                dto.setLinkReuniao(null);

                Sessao resultado = agendamentoService.agendar(mentorado, dto);
                assertEquals("Rua Teste, 10", resultado.getEndereco());
                assertNull(resultado.getLinkReuniao());
        }
